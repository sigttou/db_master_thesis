...
applied to the ELF file. As of now, there exists no known attack which applies
rowhammer to storage devices. Most operating systems use a disk cache. Meaning
the OS will keep file content in DRAM as long as the memory is not used
elsewhere. When a file is opened the system loads the content to the memory,
which in case of loading an ELF would put the whole file into the DRAM, making
it possible to apply bitflips. Therefore a later call to the program would load
the flipped version already cached in DRAM.
%}}}
