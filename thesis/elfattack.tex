...
uses.
  ...
  \item ELF files for each of the bitflips are generated.
...
defined in- and output-interface. Each part can be adopted or easily used for
...
the layout of the framework in the file system. The configuration file defines
where to find each of the required files. The template chroot describes the
...
\subsubsection{Using an AFL-like Approach}
...
Testing the \texttt{sudo} program for exploitable bitflips took about \num{8}
...
We want to find all bitflips which allow bypassing of a setup where login
