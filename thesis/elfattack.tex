...
For \texttt{sudo}, we show the same exploitable \SI{29} bitflips as
Gruss~\etal~\cite{flipinthewall} in the \texttt{sudoers} library. Besides the
\SI{29} bitflips, we show \SI{230} additional exploitable bitflips in the
library and also can report multiple other bitflips to achieve the same exploit
as Gruss~\etal~\cite{flipinthewall} with the automated bitflip search.
...
Besides that, we can report that some bitflips can exploit multiple
...
Table~\ref{tab:loginsudo} show the shared bitflips between the two applications.
...
one in the \texttt{ld-linux-x86-64.so.2} file. The change introduced by bitlfip
is shown in Listing~\ref{lst:sudosshld}. The bitflip changes the behaviour of
the linker library and because of this change the outcome of the two programs
lead to a successful state.
%}}}
