...
Processing units (PU) execute programs. Usually, the central processing unit
(CPU) runs programs. Often referred to as the processor, the CPU carries out the
instruction given to it by the program. The CPU can only step over single
instructions and execute them. This is done by moving the so-called instruction
pointer to different memory locations. Simple processors execute just one
program. On ordinary desktop computers, the operating system is the main program
being able to load other processes and manage them. CPUs can only execute
machine code, but programs usually are written in programming languages which
then are translated to machine code by either a compiler or an interpreter.
Files created by a compiler are mostly referred to as executables or binaries,
these can directly be loaded into memory by the OS. Operating Systems use
defined structures for executables to make the loading possible, for example,
Unix/Linux systems use ELF and Windows uses PE.

\subsection{CPU Instructions}

An instruction consists of an operation code (opcode), telling the
...
The Intel manual \todo{reference Intel manual} describes the instruction format
as seen in table~\ref{tab:instrfor}.Whereas the following sections can be seen:
...
We refer to the Intel Manual \todo{reference Intel Manual} section 2.1 for 
further details on the instruction format used in \texttt{x86\_64}.
...
\todo{describe sections in more details}
