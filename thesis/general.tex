...
\texttt{Apache} and \texttt{nginx}~\todo{reference webserver software}. Web
...
\subsection{HTTP - Basic Access Authentication}
...
\subsubsection{Access control in \texttt{Apache} and \texttt{NGINX}}
...
details \todo{reference ht documentation}. To make use of BA, Apache introduced
so-called \texttt{htpasswd}-files, which store user credentials. NGINX later
adapted these files for their server. The \texttt{htpasswd}-files keep the data
either in plaintext or hashed. To create these files \texttt{Apache} provides
the tool \texttt{htpasswd}\todo{reference htpasswd man page}. This tool uses MD5
