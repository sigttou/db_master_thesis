...
The schematic of AES-GCM can be seen in figure~\ref{fig:aesgcm}. Whereas we use
close to the same notation as Hanno Böck~\etal in their
paper~\cite{gcmnonceattack}:

\begin{itemize}
  \item[$CNT_i$] The $i$-th counter block, computed using concatenation of the
$IV$ and the counter value $cnt$, $cnt = (i+1) \mod{2^{32}}$, to achieve 128
...
initialisation vector with a size of 96 bits.
  ...
  \item[$a \oplus b$] XOR operation bewteen $a$ and $b$.
...
\begin{enumerate}
  \item The initialization $IV$ of 96 bits is generated.
...
by encrypting 128 zero bits with the AES block cipher, $C$ is the ciphertext
...
add the known tag $TAG$ to each polynom to get $g_i'(X)$ as follows:
...
Which is know to fulfill the equation $g_{1+2}'(H) = 0$. Therefore an attacker
